...
const typeNum = (arr) => {
  const resultArr = arr.filter((elem) => typeof elem === 'number');
  return resultArr;
};
...
const containsAnd = (arr) => {
  const resultArr = arr.filter((elem) => elem.includes('and'));
  return resultArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const resultArr = arr.filter((elem) => !forbiddenValues.includes(elem));
  return resultArr;
};
